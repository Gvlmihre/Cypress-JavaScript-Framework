 ...
 *
 * Created at     : 2023-06-06 10:17:34
 */
...
describe('Run Connector Pro V2 Public API Processing Of Sold Products Category 5.1 Tests', () => {
    ...
    it('Connector Pro V2 Public API Processing Of Sold Products 5.1 Create New Emission Source POST Request Test', () => {
        ...
        const co2EmissionFactor = faker.mersenne.rand(10, 1);
            cy.apiRequest('POST',
                `/emission-source/processing-stage`,
                {
    ...
    it('Connector Pro V2 Public API Processing Of Sold Products 5.1 GET Emission Sources Test', () => {
            cy.apiRequest('GET', `/emission-source/processing-stage?`)
                .then(json => {
                    expect(json.status).to.equals(200)
                    cy.get(json.body.content).each(item => {
    ...
    it('Connector Pro V2 Public API Processing Of Sold Products 5.1 GET Emission Sources Filter By String Test', () => {
        const string = getRandomArrayElement(emissionSources)
            cy.apiRequest('GET', `/emission-source/processing-stage?search=${string}`)
                .then(json => {
                    cy.log(json)
                    expect(json.status).to.equals(200)
                    cy.get(json.body.content).each(item => {
    ...
    it('Connector Pro V2 Public API Processing Of Sold Products 5.1 GET Calculated Carbon Footprints',() => {
        const locationId = getRandomArrayElement(locationIds)
            cy.apiRequest('GET', `/calculation/processing-stage?locationId=${locationId}`)
        .then(json => {
            expect(json.status).to.equals(200)
        })
    })

    it('Connector Pro V2 Public API Processing Of Sold Products 5.1 Calculate Carbon Footprint POST Request Test', () =>{
        const locationId = getRandomArrayElement(locationIds)
    ...
    it('Connector Pro V2 Public APIProcessing Of Sold Products 5.1 Calculate Carbon Footprint POST Request Test - 400 Bad Request', () =>{
        const locationId = getRandomArrayElement(locationIds)
        const amount = faker.mersenne.rand(1000, 1)
